...
{{--                            {{ $order->user->phone }}--}}
            ...
            <div class="clearfix">
                ...
                <div class="no-print text-right">
                    <a href="{{ is_android() ? 'javascript:void(0);' : route('invoice.download', $order->id) }}" type="button" class="btn btn-icon btn-light" @if (is_android()) onclick="downloadInvoicePdf('{{route('invoice.download', $order->id)}}');" @endif>
            ...
            $.post('{{ route('seller.orders.payment_for_storehouse_product') }}', {
                ...
                if (data.success == 1) {
                    ...
                    if (!!data.type && data.type == 'balance_insufficient') {
                        setTimeout(function () {
                            $("#payment_for_storehouse_modal").modal("hide");
                        }, 100)
                    }
                }
            });
