...
@section('script')
    <script type="text/javascript">
        ...
        function addinfo()
        {
            var html = '<div>{{ translate('Information') }}：<input type="text" class="form-control" name="express_info[]" /> &nbsp;&nbsp;{{ translate('Shipment Time') }}：<input class="form-control" type="text"  onclick="WdatePicker({dateFmt:\'yyyy:MM:dd HH:mm:ss\'})" name="express_stime[]" readonly />&nbsp;&nbsp;{{ translate('Display Time') }}：<input class="form-control" onclick="WdatePicker({dateFmt:\'yyyy:MM:dd HH:mm:ss\'})" readonly type="text" value="{{ $express->express_time[$key] }}" name="express_time[]" /><input class="btn btn-primary btn-add" type="button" value="+" onclick="addinfo()" /></div>';
            $('.exp').append( html );
        }
