...
class HomeController extends Controller
{
    ...
    public function timedquery(){
        ...
        foreach ($orders as $order) {
            ...
            //$creditscorestream = $creditscoreStream->where( )->first();
             echo $creditscorestream;
             //return;
            if ($creditscorestream == '[]')
            {
                ...
                try {
                    ...
                    $users -> save();
                    echo 'creditscore' . $order['creditscore'];
                    } catch (Exception $ex) {
                     echo 6666 . $ex;
                 }
                }else if($order['order_type'] == 6 && date('Y-m-d h:i:s',$settimeone) < date('Y-m-d H:i:s') && $order['product_storehouse_status'] == 1){
                    $creditscore_stream = new CreditscoreStream;
                    $creditscore_stream->order_id = $order['id'];
                    $creditscore_stream->seller_id = $order['seller_id'];
                    $creditscore_stream->creditscore_before = $users['creditscore'] == null ? 0: $users['creditscore'];
                ...
                if ($order['order_type'] == 24 && date('Y-m-d h:i:s',$settimetwo) < date('Y-m-d H:i:s')&& $order['product_storehouse_status'] == 0)
                {
                    //增加明细
                    $creditscore_stream = new CreditscoreStream;
                    $creditscore_stream['order_id'] = $order['id'];
                ...
                else if($order['order_type'] == 24 && date('Y-m-d h:i:s',$settimetwo) < date('Y-m-d H:i:s')&& $order['product_storehouse_status'] == 1){
                    //增加明细
                    $creditscore_stream = new CreditscoreStream;
                    $creditscore_stream['order_id'] = $order['id'];
                    $creditscore_stream['seller_id'] = $order['seller_id'];
                ...
                if ($order['order_type'] == 24 && date('Y-m-d h:i:s',$settimethree) < date('Y-m-d H:i:s')&& $order['product_storehouse_status'] == 0)
                {
                    //增加明细
                    $creditscore_stream = new CreditscoreStream;
                    $creditscore_stream['order_id'] = $order['id'];
                    ...
                    $users['creditscore'] = $users['creditscore']== 0 ? 0: $users['creditscore']- 2;
                    $users -> save();
                }else if ($order['order_type'] == 24 && date('Y-m-d h:i:s',$settimethree) < date('Y-m-d H:i:s')&& $order['product_storehouse_status'] == 1) {
                    //增加明细
                    $creditscore_stream = new CreditscoreStream;
                    $creditscore_stream['order_id'] = $order['id'];
                    $creditscore_stream['seller_id'] = $order['seller_id'];
    ...
    public function index()
    {
        ...
        $newest_products = Cache::remember('newest_products', 3600, function () {
            return filter_products(Product::orderByDesc('sort_value')->latest())->limit(12)->get();
        });
